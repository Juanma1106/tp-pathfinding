...
class Grid:
    ...
    def heuristic(self, current_state: tuple[int, int], goal_state: tuple[int, int]) -> float:
        """Heuristic function for Greedy Best-First Search

        Args:
            current_state (tuple[int, int]): Current state
            goal_state (tuple[int, int]): Goal state
